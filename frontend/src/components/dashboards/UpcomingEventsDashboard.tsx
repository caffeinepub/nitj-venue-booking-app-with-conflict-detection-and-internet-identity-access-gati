...
import { useAllUpcomingBookings } from '@/hooks/useAllUpcomingBookings';
...
export default function UpcomingEventsDashboard() {
  const { data: bookings, isLoading, isError } = useAllUpcomingBookings();
      ...
      <Card>
        <CardContent className="py-12">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">Approved bookings from today onward will appear here</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{bookings.length} event{bookings.length !== 1 ? 's' : ''} scheduled</p>
        </div>
      </div>
